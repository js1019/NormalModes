...
2001 format (2I16,1D20.10)
  ...
  subroutine CGFSE3D_ISO()
    ...
    ! for rotation
    real(kind=rkind), dimension(3)                      :: anglar
    real(kind=rkind), dimension(3,3)                    :: O33
    
    ! added JS 052319 for a constant rotation
    anglar(:) = 0.0D0; anglar(3) = 1.0D0;
    anglar = anglar/pin%rotperiod*2.0D0*PI   
    O33 = 0.0D0;
    O33(1,2) = - anglar(3)
    O33(1,3) =   anglar(2)
    O33(2,1) =   anglar(3)
                   ...
                   do n = 1,pin%s%pNp
                      ...
                      do q = 1,3
                         ...
                         if (pin%rotation) then 
                            CGM%RT%val(CGM%RT%rowdist(j)+colid) = &
                            CGM%RT%val(CGM%RT%rowdist(j)+colid) + &
                            MM((p-1)*pin%s%pNp+m,(p-1)*pin%s%pNp+n)*O33(p,q) 
                         endif 
                      enddo
                   enddo
                   deallocate(ntmp)
                   ...
                   do n = 1,pin%s%pNp
                      ...
                      do q = 1,3
                         ...
                         if (pin%rotation) then 
                            CGM%RT%val(CGM%RT%rowdist(j)+colid) = &
                            CGM%RT%val(CGM%RT%rowdist(j)+colid) + &
                            FM((p-1)*pin%s%pNp+m,(p-1)*pin%s%pNp+n)*O33(p,q) 
                         endif 
                      enddo
                   enddo
                   deallocate(ntmp)
  ...
  subroutine matrixstruct()
    ...
    ! added JS 052319 for RT matrix structure
    ! for matrix RT 
    if (pin%rotation) then 
       allocate(CGM%RT%rnum(unstrM%new%nvtx))
       CGM%RT%rnum = CGM%A%rnum
       CGM%RT%siz  = CGM%A%siz 
       CGM%RT%Gsiz = CGM%A%Gsiz 
       allocate(CGM%RT%sizdist(unstrM%nproc+1))
       CGM%RT%sizdist = CGM%A%sizdist
       allocate(CGM%RT%rstt(unstrM%new%nvtx))
       CGM%RT%rstt = CGM%A%rstt
       allocate(CGM%RT%rowdist(CGM%RT%siz+1))
       CGM%RT%rowdist = CGM%A%rowdist
       CGM%RT%NNZ     = CGM%RT%rowdist(CGM%RT%siz+1)
       allocate(CGM%RT%col(CGM%A%NNZ),CGM%RT%val(CGM%A%NNZ))
       CGM%RT%col = CGM%A%col; CGM%RT%val = 0.0D0;
       CGM%RT%GNNZ = CGM%A%GNNZ
    endif 
  ...
  subroutine matrixstruct_general()
    ...
    if (pin%rotation) then 
       allocate(CGM%RT%rnum(unstrM%new%nvtx))
       CGM%RT%rnum = CGM%Ad%rnum
       CGM%RT%siz  = CGM%Ad%siz 
       CGM%RT%Gsiz = CGM%Ad%Gsiz 
       allocate(CGM%RT%sizdist(unstrM%nproc+1))
       CGM%RT%sizdist = CGM%Ad%sizdist
       allocate(CGM%RT%rstt(unstrM%new%nvtx))
       CGM%RT%rstt = CGM%Ad%rstt
       allocate(CGM%RT%rowdist(CGM%RT%siz+1))
       CGM%RT%rowdist = CGM%Ad%rowdist
       CGM%RT%NNZ     = CGM%RT%rowdist(CGM%RT%siz+1)
       allocate(CGM%RT%col(CGM%Ad%NNZ),CGM%RT%val(CGM%Ad%NNZ))
       CGM%RT%col  = CGM%Ad%col; CGM%RT%val = 0.0D0;
       CGM%RT%GNNZ = CGM%Ad%GNNZ
    endif 
