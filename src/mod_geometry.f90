...
module geometry_mod
 ...
 !--------------------------------------------------------------------
  ...
  subroutine pnm_loc_data_build()
    ...
    if (pin%buildtetJac) then 
       ...
       do i = 1, unstrM%ClNele
          ...
          !! check
          !if (abs(unstrM%loc_detJ(i)).le.1.D-10) then 
          !   print*, unstrM%loc_detJ(i), i, unstrM%rank
          !endif 
       enddo
       if (unstrM%errJ) then 
          print*, 'Error: Something is wrong with Jacobian'
          stop
       endif  
    endif
  ...
  subroutine transfMat(tetidx,B,Binv,detB)
    ...
    if (abs(detB).le.1.D-20) then
      print*, 'Error: the ', i+1,'-th tetrahedron is (almost) flat'
      unstrM%errJ = .true.
    elseif (detB.le. -1.D-20) then 
      print*, 'Error: negative Jacobian at local ele',i+1,'on process',unstrM%rank
      unstrM%errJ = .true.
    endif
